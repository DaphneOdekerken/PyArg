...
def get_relevant_uncertain_arguments_for_removing_from_pg(
    ...
    def rel_given_pg(argument_from: Argument) -> List[Argument]:
        if argument_from not in pg:
            return []
        if not argument_from.get_ingoing_attack_arguments:
            return []
        attackers_to_add = [att for att in argument_from.get_ingoing_attack_arguments
                            ...
                            not any(att_att.name in incomplete_argumentation_framework.arguments.keys()
                                    and att not in att_att.get_ingoing_attack_arguments
                                    for att_att in att.get_ingoing_attack_arguments)]
        if not attackers_to_add:
            attackers_to_add = [relevant_argument
                                for att in argument_from.get_ingoing_attack_arguments
                                for att_att in argument_from.get_ingoing_attack_arguments
                                if att not in att_att.get_ingoing_attack_arguments
