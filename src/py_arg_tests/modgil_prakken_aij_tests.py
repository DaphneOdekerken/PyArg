...
from py_arg.aspic.classes.orderings.argument_orderings.last_link_ordering \
    import LastLinkElitistOrdering, LastLinkDemocraticOrdering
...
from py_arg.aspic.classes.orderings.argument_orderings.weakest_link_ordering \
    import WeakestLinkElitistOrdering, WeakestLinkDemocraticOrdering
from py_arg.aspic.classes.instantiated_argument import InstantiatedArgument


def get_argumentation_theory(
        include_d: bool = False, include_e: bool = False) -> \
        ArgumentationTheory:
    ...
    contraries_and_contradictories = {literal_str: []
                                      for literal_str in language.keys()}
    for literal_str in language.keys():
        if literal_str[0] in ('~', '-'):
            contraries_and_contradictories[literal_str].append(
                language[literal_str[1:]])
        else:
            contraries_and_contradictories[literal_str].append(
                language['-' + literal_str])

    strict_rules = [StrictRule('s1', {language['t'], language['q']},
                               language['-p'])]
    ...
    if include_d:
        defeasible_rules.append(
            DefeasibleRule('d4', {language['~d']}, language['s']))
    ...
    for defeasible_rule in defeasible_rules:
        ...
        language[str(defeasible_rule_literal_negation)] = \
            defeasible_rule_literal_negation
        contraries_and_contradictories[str(defeasible_rule_literal)] = \
            [defeasible_rule_literal_negation]
        contraries_and_contradictories[
            str(defeasible_rule_literal_negation)] = [defeasible_rule_literal]

    if include_e:
        strict_rules.append(StrictRule('s2', {language['r']}, language['-d3']))

    arg_sys = ArgumentationSystem(language, contraries_and_contradictories,
                                  strict_rules, defeasible_rules)

    axioms = []
    ordinary_premises = [language[literal_str]
                         for literal_str in ['a', 'r', '-r', '~s']]
...
class TestModgilPrakkenAIJ(unittest.TestCase):
    """
    This class tests the running example from: S.J. Modgil & H. Prakken,
    A general account of argumentation with
    ...
    def test_arguments(self):
        ...
        arg_a_prime = InstantiatedArgument.ordinary_premise_based(
            language['a'])
        arg_a = InstantiatedArgument.defeasible_rule_based(
            DefeasibleRule(
                'd3', {language['a']}, language['p']), {arg_a_prime})
        ...
        arg_b = InstantiatedArgument.strict_rule_based(
            StrictRule('s1', {language['t'], language['q']}, language['-p']),
            {arg_b2_prime, arg_b1_prime})
        arg_c = InstantiatedArgument.ordinary_premise_based(language['-r'])

        self.assertListEqual(sorted(all_args),
                             sorted([arg_a_prime, arg_a, arg_b1, arg_b1_prime,
                                     arg_b2, arg_b2_prime, arg_b, arg_c]))
        ...
        self.assertSetEqual(arg_b.sub_arguments, {arg_b1, arg_b2, arg_b1_prime,
                                                  arg_b2_prime, arg_b})
        self.assertEqual(arg_b.top_rule,
                         StrictRule('s1', {language['t'], language['q']},
                                    language['-p']))
        self.assertSetEqual(arg_b.defeasible_rules, {
            DefeasibleRule('d1', {language['~s']}, language['t']),
            DefeasibleRule('d2', {language['r']}, language['q'])
        })
        self.assertSetEqual(arg_b.strict_rules, {StrictRule(
            's1', {language['t'], language['q']}, language['-p'])})
        ...
        arg_d2 = InstantiatedArgument.defeasible_rule_based(DefeasibleRule(
            'd4', {language['~d']}, language['s']), {arg_d1})
        ...
        arg_e = InstantiatedArgument.strict_rule_based(
            arg_theory.argumentation_system.strict_rules[1], {arg_b2})
        ...
        arg_theory.ordinary_premise_preferences.append(
            (language['-r'], language['r']))
        arg_theory.ordinary_premise_preferences.append(
            (language['~s'], language['-r']))

        self.assertSetEqual(arg_a.defeasible_rules,
                            arg_a.last_defeasible_rules, {d3})
        self.assertSetEqual(arg_a.ordinary_premises, {language['a']})
        self.assertSetEqual(arg_a_prime.defeasible_rules,
                            arg_a_prime.last_defeasible_rules, set())
        self.assertSetEqual(arg_a_prime.ordinary_premises, {language['a']})
        self.assertSetEqual(arg_b.defeasible_rules,
                            arg_b.last_defeasible_rules, {d1, d2})
        self.assertSetEqual(arg_b.ordinary_premises,
                            {language['~s'], language['r']})
        self.assertSetEqual(arg_b2.defeasible_rules,
                            arg_b2.last_defeasible_rules, set())
        ...
        self.assertTrue(eli.rule_set_is_strictly_weaker_than(
            arg_b.last_defeasible_rules, arg_a.last_defeasible_rules))
        ell = LastLinkElitistOrdering(
            arg_theory.argumentation_system.rule_preferences,
            arg_theory.ordinary_premise_preferences)
        self.assertTrue(ell.argument_is_strictly_weaker_than(arg_b, arg_a))
        self.assertTrue(eli.rule_set_is_strictly_weaker_than(
            arg_b.defeasible_rules, arg_a.defeasible_rules))
        self.assertFalse(eli.ordinary_premise_set_is_strictly_weaker_than(
            arg_b.ordinary_premises, arg_a.ordinary_premises))
        ewl = WeakestLinkElitistOrdering(
            arg_theory.argumentation_system.rule_preferences,
            arg_theory.ordinary_premise_preferences)
        ...
        arg_d1 = InstantiatedArgument.ordinary_premise_based(language['~d'])
        arg_d2 = InstantiatedArgument.defeasible_rule_based(
            DefeasibleRule('d4', {language['~d']}, language['s']), {arg_d1})
        arg_e = InstantiatedArgument.strict_rule_based(
            arg_theory.argumentation_system.strict_rules[1], {arg_b2})
