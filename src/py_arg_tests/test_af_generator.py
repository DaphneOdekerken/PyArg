...
from py_arg.generators.abstract_argumentation_framework_generators.abstract_argumentation_framework_generator import \
    AbstractArgumentationFrameworkGenerator, ErdosRenyiGenerator, WattsStrogatz, BarabasiAlbert
...
class TestAFGenerator(unittest.TestCase):
    ...
    def test_wats_strogatz_generator(self):
        generator = WattsStrogatz(nr_of_arguments=10,nr_join_neighbours=5, prob_edge_rewiring=0.5,seed=42,prob_cycle=0.8)
        af = generator.generate()
        self.assertEqual(len(af.arguments), 10)
