...
import py_arg.abstract_argumentation.canonical_constructions. \
    check_union_closed as check_union_closed
...
from py_arg.assumption_based_argumentation.semantics import \
    get_complete_extensions as get_complete_extensions
from py_arg.assumption_based_argumentation.semantics import \
    get_conflict_free_extensions as get_conflict_free_extensions, \
    get_stable_extensions as get_stable_extensions, get_admissible_extensions \
    as get_admissible_extensions
...
class TestCanonicalConstructions(unittest.TestCase):
    ...
    def test_instantiation(self):
        ...
        aba = AssumptionBasedArgumentationFramework(assumptions, rules, atoms,
                                                    contraries)
        af = aba.generate_af()
    ...
    def test_canonical_stb(self):
        ...
        self.assertEqual(canonical_aba2.rules, {
            Rule('', {a}, b_c), Rule('', {a}, c_c),
            Rule('', {b}, a_c), Rule('', {b}, c_c),
            Rule('', {c}, a_c), Rule('', {c}, b_c)})
        self.assertEqual(canonical_aba2.contraries, {a: a_c, b: b_c, c: c_c})
        self.assertEqual(
            get_stable_extensions.get_stable_extensions(canonical_aba2), es2)
    ...
    def test_canonical_adm(self):
        ...
        es_n = get_admissible_extensions.get_admissible_extensions(
            canonical_aba1)
