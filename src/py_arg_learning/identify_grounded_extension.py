...
class IdentifyGroundedExtension(AbstractExerciseSet):
    def get_explanation_html(self):
        ...
        return "The grounded extension is the minimal complete extension " \
               "w.r.t. set inclusion."

    def generate_exercise_and_solutions(self, color_blind_mode: bool):
        argumentation_framework_generator = \
            AbstractArgumentationFrameworkGenerator(5, 4, True)
        ...
        grounded_extension_argument_names = [
            argument.name
            for argument in grounded_extension]
    ...
    def render_exercise_instance_text(self, exercise_instance):
        ...
        exercise_text = 'Consider an abstract argumentation framework AF = ' \
                        '(A, R) with A = {{{a}}} and R = {{{r}}}. ' \
                        'Which arguments belong to the grounded extension ' \
                        '(divide by commas)?'.format(
    ...
    def get_feedback(self, solution_by_user: str,
        ...
        if solution_learner == ground_truth:
            ...
            return 'This is not correct. The right answer was ' + \
                pre_generated_solutions[0] + '.'
