...
class ListCompleteExtensions(AbstractExerciseSet):
    def get_explanation_html(self):
        return "A complete extension is a set of arguments that contains " \
               "exactly those arguments that it defends."
    ...
    def generate_exercise_and_solutions(self, color_blind_mode: bool):
        ...
        exercise_instance = self.argumentation_framework_to_exercise_instance(
            af)
    ...
    def render_exercise_instance_text(self, exercise_instance):
        ...
        exercise_text = 'Consider an abstract argumentation framework ' \
                        ...
                        'List all complete extensions. Use a new line for ' \
                        'each set, and ' \
    ...
    def get_feedback(self, solution_by_user: str,
        ...
        solutions_learner_parts = [preprocess(part)
                                   for part in solutions_learner.split('\n')]
        ground_truth_parts = [preprocess(part)
                              for part in pre_generated_solutions]

        overlapping_parts = [part for part in solutions_learner_parts
        ...
        if len(overlapping_parts) < len(solutions_learner_parts):
            ...
            return 'The following parts of your answer were ' \
                   'not complete extensions:\n' + '\n'.join(not_complete)
