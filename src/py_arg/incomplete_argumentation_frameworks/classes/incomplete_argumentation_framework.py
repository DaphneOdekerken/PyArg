...
class IncompleteArgumentationFramework:
    """
    An incomplete argumentation framework (IAF) is an extension to abstract
    argumentation frameworks
    in which the sets of arguments and defeats are each split in two disjoint
    parts: a certain part
    ...
    def __init__(self, name: str = '',
        ...
        if uncertain_arguments is None:
            ...
            if any(uncertain_argument in arguments
                   for uncertain_argument in uncertain_arguments):
                raise ValueError('Argument cannot be both certain and '
                                 'uncertain.')
            self._uncertain_arguments = {argument.name: argument
                                         for argument in uncertain_arguments}
    ...
    def __repr__(self):
        return '( [' + ', '.join(
            ...
            '], [' + ', '.join(
                argument_key
                for argument_key in self.uncertain_arguments.keys()) + \
            '], [' + ', '.join(
                defeat.__repr__()
                for defeat in self.defeats) + \
            '], [' + ', '.join(
                defeat.__repr__()
                for defeat in self.uncertain_defeats) + '] )'
    ...
    def certain_projection(self) -> AbstractArgumentationFramework:
        """
        The certain projection consists of all certain arguments and all
        certain defeats between two
        certain arguments.
        """
    ...
    def get_all_completions(self) -> List[AbstractArgumentationFramework]:
        """
        The set of completions consists of all abstract AFs that can be
        obtained by resolving the
        ...
        remaining_uncertain_argument_options = powerset(
            self.uncertain_arguments.values())
        remaining_uncertain_defeat_options = powerset(self.uncertain_defeats)
        combined_options = itertools.product(
            remaining_uncertain_argument_options,
            remaining_uncertain_defeat_options)
        ...
        for uncertain_arguments, uncertain_defeats in combined_options:
            ...
            completion = AbstractArgumentationFramework(
                ...
                defeats=[defeat for defeat in all_defeats
                         if defeat.to_argument in all_arguments and
                         defeat.from_argument in all_arguments]
            )
    ...
    def get_all_partial_completions(self) -> \
            List['IncompleteArgumentationFramework']:
        """
        The set of partial completions consists of all IAFs that can be
        obtained by resolving some of the
        ...
        result = []
        for uncertain_arguments, uncertain_defeats in combined_options:
            # This is a particular guess for the uncertain elements that
            # will remain (as either certain or uncertain
            # arguments) in the partial completion.

            # Now construct all possible combinations of these remaining
            # elements.
            new_certain_argument_options = powerset(uncertain_arguments)
            new_certain_defeat_options = powerset(uncertain_defeats)
            new_combined_options = itertools.product(
                new_certain_argument_options, new_certain_defeat_options)

            for new_certain_arguments, new_certain_defeats in \
                    new_combined_options:
                # This is a particular guess for the elements that will become
                # the new certain elements in the partial
                # completion.
                all_new_certain_arguments = \
                    sorted(old_certain_arguments + list(new_certain_arguments))
                all_new_certain_defeats = \
                    sorted(old_certain_defeats + list(new_certain_defeats))
                new_uncertain_arguments = \
                    sorted([arg for arg in uncertain_arguments
                            if arg not in new_certain_arguments])
                new_uncertain_defeats = \
                    sorted([defeat for defeat in uncertain_defeats
                            if defeat not in new_certain_defeats])

                all_new_certain_or_uncertain_arguments = \
                    all_new_certain_arguments + new_uncertain_arguments
                ...
                partial_completion = IncompleteArgumentationFramework(
                    ...
                    defeats=[defeat for defeat in all_new_certain_defeats
                             if defeat.to_argument in
                             all_new_certain_or_uncertain_arguments and
                             defeat.from_argument in
                             all_new_certain_or_uncertain_arguments],
                    uncertain_defeats=[
                        defeat
                        for defeat in new_uncertain_defeats
                        if defeat.to_argument in
                        all_new_certain_or_uncertain_arguments and
                        defeat.from_argument in
                        all_new_certain_or_uncertain_arguments]
