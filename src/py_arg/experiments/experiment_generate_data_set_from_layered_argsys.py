...
def instantiate_incomplete_argumentation_theory_generator(
    ...
    arg_sys, layered_language = \
        layered_argumentation_system_generator.generate(
            return_layered_language=True,  add_rule_preferences=True)
    ...
    topics = list({str(literal)
                   for literal in layered_language[3] + layered_language[4]})
        ...
        for arg_sys_nr in range(25):
            ...
            for iat_nr in range(25):
                ...
                IncompleteArgumentationTheoryToLPFileWriter().write(
                    ...
                    file_name=f'DS2_{str(literal_size)}L{str(rule_size)}'
                              f'R_AS{str(arg_sys_nr)}_IAT{str(iat_nr)}.pl',
