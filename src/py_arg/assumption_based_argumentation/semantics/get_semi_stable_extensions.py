...
# cf. 'ON THE DIFFERENCE BETWEEN ASSUMPTION-BASED ARGUMENTATION AND ABSTRACT
# ARGUMENTATION'
def get_semi_stable_extensions(
    ...
    for ext in com_ext:
        ...
        for arg in af.arguments:
            if arg.premise.issubset(ext) and \
                    arg.conclusion not in aba_framework.assumptions:
                conclusion_index = \
                    list(aba_framework.contraries.values()).index(
                        arg.conclusion)
                extension_reach[ext].add(list(
