...
class IncompleteArgumentationTheoryFromXLSXFileReader:
    ...
    @staticmethod
    def read_from_xlsx_file(file_path: Union[Path, str]) -> \
            IncompleteArgumentationTheory:
        ...
        # _query_df = pd.read_excel(file_path, sheet_name='Queries',
        #                           index_col='Query')
        ...
        #     _rule_preference_df = pd.read_excel(file_path,
        #                                         sheet_name='RulePreferences')
        # else:
        ...
        try:
            ...
            if _contrary_df is not None:
                for index, row in _contrary_df.iterrows():
                    ...
                    contraries_str = [cont.strip().replace('~', '-')
                                      for cont in row['Contraries'].split(',')]
                    for contrary_str in contraries_str:
                        contraries_and_contradictories[literal_str].add(
                            language[contrary_str])

            # Add rules
            rules = []
            for index, row in _rule_df.iterrows():
                ants_str = [ant.strip().replace('~', '-')
                            for ant in row['Antecedents'].split(',')]
                cons_str = row['Consequent'].strip().replace('~', '-')
                if any([ant not in language
                        for ant in ants_str]) or cons_str not in language:
                    raise KeyError(
                        'Not each literal in ' + str(ants_str) + ' or ' +
                        cons_str + ' was in logical language.')
            ...
            raise ImportError('Could not load Argumentation System.' +
                              str(type(exception)))
