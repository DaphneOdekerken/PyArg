...
class IncompleteArgumentationTheoryToLPFileWriter(Writer):
    ...
    def write(
        ...
        with open(write_path, 'w') as write_file:
            for queryable in \
                    incomplete_argumentation_theory.positive_queryables:
            ...
            for literal in incomplete_argumentation_theory.\
                ...
                if literal.s1[0] != '-':
                    for contrary in literal.contraries_and_contradictories:
                        write_file.write(f'neg({literal.s1.lower()}, '
                                         f'{contrary.s1.lower()}).\n')
            write_file.write('\n')
            ...
            for rule in incomplete_argumentation_theory.\
                ...
                for antecedent in rule.antecedents:
                    write_file.write(f'body({str(id_rule)}, '
                                     f'{antecedent.s1.lower()}).\n')
                write_file.write(f'head({str(id_rule)}, '
                                 f'{rule.consequent.s1.lower()}).\n')
            write_file.write('\n')

            for (r1, r2) in incomplete_argumentation_theory.\
