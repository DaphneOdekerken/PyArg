...
class StabilityLabel:
    def __init__(self, unsatisfiable: bool, defended: bool, out: bool,
                 blocked: bool):
    ...
    def __str__(self):
        s1 = '(U:{0}, D:{1}, O:{2}, B:{3})'.format(
            str(self.unsatisfiable), str(self.defended), str(self.out),
            str(self.blocked))
        return s1
    ...
    def __eq__(self, other):
        return self.unsatisfiable == other.unsatisfiable and \
            self.defended == other.defended and \
            self.out == other.out and self.blocked == other.blocked

    def __copy__(self):
        return StabilityLabel(self.unsatisfiable, self.defended,
                              self.out, self.blocked)
    ...
    @property
    def is_stable(self):
        return sum([self.unsatisfiable, self.defended, self.out,
                    self.blocked]) == 1
    ...
    def is_contested_stable(self):
        """
        Contested is an alternative stability status, using either the defended
        justification status or combining
        the justification statuses that are not defended. So if some literal is
        contested-stable, then it is either
    ...
    def from_str(cls, label_str: str):
        parse_result_str_list = parse(
            '(U:{0}, D:{1}, O:{2}, B:{3})', label_str).fixed
