...
class FourBoolRelevanceLister:
    ...
    def update(
        ...
        # Start by collecting relevant observables of leaves (or-nodes for
        # which there is no rule) and observables
        ...
        while nodes_to_update_relevancy_round:
            ...
            for literal in nodes_to_update_relevancy_round:
                old_relevancy = self._get_current_relevancy_for_literal(
                    literal)
                self._update_relevant_literals_for_literal(
                    incomplete_argumentation_theory,
                    literal, incomplete_argumentation_theory.knowledge_base)
                new_relevancy = self._get_current_relevancy_for_literal(
                    literal)
                if old_relevancy != new_relevancy:
                    new_rules_to_update_relevancy = \
                        new_rules_to_update_relevancy | \
                        set(literal.parents)
            ...
            for rule in nodes_to_update_relevancy_round:
                ...
                if old_relevancy != new_relevancy:
                    new_literals_to_update_relevancy = \
                        new_literals_to_update_relevancy | \
                        set([rule.consequent] + list(
                            rule.consequent.contraries_and_contradictories))
            nodes_to_update_relevancy_round = new_literals_to_update_relevancy

        for literal in literals:
            self.relevance_list[
                literal] = self._get_current_relevancy_for_literal(literal)

    def _get_current_relevancy_for_literal(self, literal: Literal):
        return self.u_relevant_literal[literal] | self.d_relevant_literal[
            literal] | \
    ...
    def _update_relevant_literals_for_literal(
            self,
            incomplete_argumentation_theory: IncompleteArgumentationTheory,
            literal: Literal, knowledge_base):
        self._update_relevant_literals_u_literal(
            incomplete_argumentation_theory, knowledge_base, literal)
        self._update_relevant_literals_d_literal(
            incomplete_argumentation_theory, knowledge_base, literal)
        self._update_relevant_literals_o_literal(
            incomplete_argumentation_theory, knowledge_base, literal)
        self._update_relevant_literals_b_literal(
            incomplete_argumentation_theory, literal)
    ...
    def _update_relevant_literals_u_literal(self,
                                            incomplete_argumentation_theory,
        ...
        if self._lit_label(literal).unsatisfiable:
            if incomplete_argumentation_theory.is_queryable(literal):
                if literal not in knowledge_base and all(
                        [contrary not in knowledge_base
                         for contrary in
    ...
    def _update_relevant_literals_d_literal(self,
                                            incomplete_argumentation_theory,
                                            knowledge_base, literal):
        d_relevant = set()
        ...
        if self._lit_label(literal).defended:
            if incomplete_argumentation_theory.is_queryable(literal):
                if literal not in knowledge_base and \
                        all([contrary not in knowledge_base for contrary in
                             literal.contraries_and_contradictories]):
                    for contrary in literal.contraries_and_contradictories:
                        if all([contrary_contrary not in knowledge_base for
                                contrary_contrary in
                                contrary.contraries_and_contradictories]):
                            d_relevant.add(contrary)
            else:
                if all([self._rule_label(
                        rule).unsatisfiable or self._rule_label(rule).out
                        or self._rule_label(rule).blocked
                        for rule in literal.children]):
                    for rule in literal.children:
                        if self._rule_label(rule).defended:
                            d_relevant = d_relevant | self.d_relevant_rule[
                                rule]

                for contrary in literal.contraries_and_contradictories:
                    for contrary_rule in contrary.children:
                        if self._rule_label(
                                contrary_rule).defended or self._rule_label(
                                contrary_rule).blocked:
                            if self._rule_label(contrary_rule).unsatisfiable:
                                d_relevant = d_relevant | self.u_relevant_rule[
                                    contrary_rule]
                            if self._rule_label(contrary_rule).out:
                                d_relevant = d_relevant | self.o_relevant_rule[
                                    contrary_rule]

        self.d_relevant_literal[literal] = d_relevant

    def _update_relevant_literals_o_literal(self,
                                            incomplete_argumentation_theory,
                                            knowledge_base, literal):
        o_relevant = set()

        if self._lit_label(literal).out:
            if all([self._rule_label(rule).unsatisfiable for rule in
                    literal.children]):
                for rule in literal.children:
                    if self._rule_label(rule).defended:
            ...
            if incomplete_argumentation_theory.is_queryable(literal):
                ...
                if all([self._rule_label(
                        rule).unsatisfiable or self._rule_label(rule).defended
                        or self._rule_label(rule).blocked
                        for rule in literal.children]):
                    for rule in literal.children:
                        o_relevant = o_relevant | self.o_relevant_rule[rule]
                for rule in literal.children:
                    if self._rule_label(rule).defended or self._rule_label(
                            rule).blocked:
                        if self._rule_label(rule).unsatisfiable:
                            o_relevant = o_relevant | self.u_relevant_rule[
                                rule]
                        if self._rule_label(rule).out:
                            o_relevant = o_relevant | self.o_relevant_rule[
                                rule]

        self.o_relevant_literal[literal] = o_relevant

    def _update_relevant_literals_b_literal(self,
                                            incomplete_argumentation_theory,
                                            literal):
        b_relevant = set()
        ...
        if self._lit_label(
                literal).blocked and \
                not incomplete_argumentation_theory.is_queryable(
                literal):
            if all([self._rule_label(rule).unsatisfiable or self._rule_label(
                    rule).out for rule in literal.children]):
                for rule in literal.children:
                    if self._rule_label(rule).defended:
            ...
            if all([self._rule_label(
                    contrary_rule).unsatisfiable or self._rule_label(
                    contrary_rule).out
                    for contrary in literal.contraries_and_contradictories for
                    contrary_rule in contrary.children]):
                if all([self._rule_label(
                        rule).unsatisfiable or self._rule_label(rule).defended
                    ...
                    for rule in literal.children:
                        if self._rule_label(rule).blocked:
                            b_relevant = b_relevant | self.b_relevant_rule[
                                rule]
                    for contrary in literal.contraries_and_contradictories:
                        for contrary_rule in contrary.children:
                            if self._rule_label(contrary_rule).defended:
                                b_relevant = b_relevant | self.d_relevant_rule[
                                    contrary_rule]
                    ...
                    if self._rule_label(rule).defended:
                        if self._rule_label(rule).unsatisfiable:
                            b_relevant = b_relevant | self.u_relevant_rule[
                                rule]
                        if self._rule_label(rule).out:
                            b_relevant = b_relevant | self.o_relevant_rule[
                                rule]
                        if self._rule_label(rule).blocked:
                            b_relevant = b_relevant | self.b_relevant_rule[
                                rule]
    ...
    def _update_relevant_literals_u_rule(self, rule):
        ...
        if self._rule_label(rule).unsatisfiable and \
                all([self._lit_label(literal).defended or self._lit_label(
                    literal).out
    ...
    def _update_relevant_literals_o_rule(self, rule):
        ...
        if self._rule_label(rule).out:
            if all([self._lit_label(literal).unsatisfiable or self._lit_label(
                    literal).defended
                    or self._lit_label(literal).blocked
                    for literal in rule.antecedents]):
                for literal in rule.antecedents:
                    if self._lit_label(literal).out:
                        o_relevant = o_relevant | self.o_relevant_literal[
                            literal]

            for literal in rule.antecedents:
                if self._lit_label(literal).unsatisfiable:
                    if self._lit_label(literal).defended:
                        o_relevant = o_relevant | self.d_relevant_literal[
                            literal]
                    if self._lit_label(literal).out:
                        o_relevant = o_relevant | self.o_relevant_literal[
                            literal]
                    if self._lit_label(literal).blocked:
                        o_relevant = o_relevant | self.b_relevant_literal[
                            literal]
    ...
    def _update_relevant_literals_b_rule(self, rule):
        ...
        if self._rule_label(rule).blocked:
            if all([self._lit_label(literal).unsatisfiable or self._lit_label(
                ...
                for literal in rule.antecedents:
                    if self._lit_label(literal).blocked:
                        b_relevant = b_relevant | self.b_relevant_literal[
                            literal]

            for literal in rule.antecedents:
                if self._lit_label(literal).unsatisfiable or self._lit_label(
                        literal).out:
                    if self._lit_label(literal).defended:
                        b_relevant = b_relevant | self.d_relevant_literal[
                            literal]
                    if self._lit_label(literal).blocked:
                        b_relevant = b_relevant | self.b_relevant_literal[
                            literal]
