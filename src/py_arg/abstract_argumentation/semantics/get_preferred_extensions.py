...
class PreferredExtensionLabel(Enum):
    ...
    BLANK = 3           # Default label for all arguments, indicating
    # that the argument is still unprocessed.
    MUST_OUT = 4        # Argument defeats IN argument
    UNDEC = 5           # Argument may not be included in a preferred
    # extension because not defended by any IN argument.
...
def get_preferred_extensions(
    ...
    return _recursively_get_preferred_extensions(argumentation_framework,
                                                 initial_labelling, set())


def _recursively_get_preferred_extensions(
        argumentation_framework: AbstractArgumentationFramework,
        labelling: Dict[Argument, PreferredExtensionLabel],
        preferred_extensions: Set[FrozenSet[Argument]]) -> \
        Set[FrozenSet[Argument]]:
    if all(labelling[argument] != PreferredExtensionLabel.BLANK
           for argument in argumentation_framework.arguments):
        ...
        preferred_extensions = _recursively_get_preferred_extensions(
...
def _in_trans(labelling: Dict[Argument, PreferredExtensionLabel],
              argument: Argument,
              argumentation_framework: AbstractArgumentationFramework) -> \
...
def _undec_trans(labelling: Dict[Argument, PreferredExtensionLabel],
                 argument: Argument) -> \
        Dict[Argument, PreferredExtensionLabel]:
    new_labelling = labelling.copy()
