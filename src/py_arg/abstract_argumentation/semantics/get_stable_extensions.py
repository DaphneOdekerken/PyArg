...
class StableExtensionLabel(Enum):
    ...
    UNDEC = 5  # Argument may not be included in a stable extension because not
...
def get_stable_extensions(
    ...
    return _recursively_get_stable_extensions(argumentation_framework,
                                              initial_labelling, set())


def _recursively_get_stable_extensions(
        argumentation_framework: AbstractArgumentationFramework,
        labelling: Dict[Argument, StableExtensionLabel],
        stable_extensions: Set[FrozenSet[Argument]]) -> \
        Set[FrozenSet[Argument]]:
    if all(labelling[argument] != StableExtensionLabel.BLANK
           for argument in argumentation_framework.arguments):
        if all(labelling[argument] != StableExtensionLabel.MUST_OUT and
               labelling[argument] != StableExtensionLabel.UNDEC
               for argument in argumentation_framework.arguments):
            stable_extensions.add(frozenset(sorted({
        ...
        alternative_labelling = _in_trans(
            labelling, blank_argument, argumentation_framework)
        stable_extensions = _recursively_get_stable_extensions(
            argumentation_framework, alternative_labelling, stable_extensions)
