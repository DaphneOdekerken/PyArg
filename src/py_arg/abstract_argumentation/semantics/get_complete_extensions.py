...
class CompleteExtensionLabel(Enum):
    ...
    UNDEC = 5  # Argument may not be included in a complete extension because
...
def get_complete_extensions(
    ...
    return _recursively_get_complete_extensions(argumentation_framework,
                                                initial_labelling, set())


def _recursively_get_complete_extensions(
        argumentation_framework: AbstractArgumentationFramework,
        labelling: Dict[Argument, CompleteExtensionLabel],
        complete_extensions: Set[FrozenSet[Argument]]) -> \
        Set[FrozenSet[Argument]]:
    if all(labelling[argument] != CompleteExtensionLabel.BLANK for argument in
           argumentation_framework.arguments):
        if all(labelling[argument] != CompleteExtensionLabel.MUST_OUT
               for argument in argumentation_framework.arguments):
            candidate_complete_extension = frozenset(sorted(
            ...
            for argument in candidate_complete_undec:
                if all(labelling[defeater] != CompleteExtensionLabel.IN for
                       defeater in
                       argumentation_framework.get_incoming_defeat_arguments(
                           argument)) and \
                        not all(labelling[
                                    defeater_out] == CompleteExtensionLabel.OUT
                                for defeater_out in
                                argumentation_framework.
                                get_incoming_defeat_arguments(
                                    argument)):
        ...
        alternative_labelling = _in_trans(labelling, blank_argument,
                                          argumentation_framework)
        complete_extensions = _recursively_get_complete_extensions(
