...
def filter_argumentation_framework_extensions_with_argument(
        all_extensions: Set[FrozenSet[T]],
        argument: T) -> Set[FrozenSet[T]]:
    """
    Select only those extensions containing the given argument.

    :param all_extensions: All extensions, computed earlier.
    :param argument: The argument that should be contained in each extension
    that is left in the result.
    """
    return {frozen_extension for frozen_extension in all_extensions
            if argument in frozen_extension}


def get_argumentation_framework_extensions_with_argument(
