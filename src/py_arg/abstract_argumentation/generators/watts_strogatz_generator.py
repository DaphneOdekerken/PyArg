...
from py_arg.abstract_argumentation.generators.abstract_argumentation_framework_generator import ALPHABET
from py_arg.abstract_argumentation.generators.generate_directed_graph import generate_directed_graph
from py_arg.abstract_argumentation.generators.enforce_prob_cycle import enforce_prob_cycle


class WattsStrogatz:
    def __init__(self, nr_of_arguments: int, prob_edge_rewiring: float, prob_cycle: float, nr_join_neighbours: int,
                 seed: int = None):
    ...
    def generate(self, name: Optional[str] = None) -> AbstractArgumentationFramework:
        ...
        watts_strogatz_graph = nx.watts_strogatz_graph(n=self.nr_of_arguments, k=self.nr_join_neighbours,
        ...
        defeats = [Defeat(node_id_to_argument_map[defeat[0]], node_id_to_argument_map[defeat[1]]) for defeat in
