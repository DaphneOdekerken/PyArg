...
class ArgumentationSystemFromJsonReader:
    ...
    def from_json(json_object) -> ArgumentationSystem:
        language = {literal_str: Literal(literal_str)
                    for literal_str in json_object['language']}
        contraries = {literal_str: {language[con_lit]
                                    for con_lit in con_lit_list}
                      for literal_str, con_lit_list
                      in json_object['contraries'].items()}

        defeasible_rules = []
        for defeasible_rule_str in json_object['defeasible_rules']:
            antecedents_str, consequent_str = defeasible_rule_str[1].split(
                '=>', 1)
            antecedent_str_list = antecedents_str.split(',')
            defeasible_rule = DefeasibleRule(
                defeasible_rule_str[0],
                {language[antecedent_str]
        ...
        for rule_preference in json_object['rule_preferences']:
            rule_preferences.append((def_rules_lookup[rule_preference[0]],
                                     def_rules_lookup[rule_preference[1]]))
