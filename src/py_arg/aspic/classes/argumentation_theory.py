...
from py_arg.aspic.classes.orderings.argument_orderings.last_link_ordering \
    import LastLinkElitistOrdering
...
from py_arg.aspic.classes.orderings.preference_preorder import \
    PreferencePreorder
from py_arg.aspic.classes.structured_argumentation_framework import \
    StructuredArgumentationFramework


class ArgumentationTheory:
    """
    An ArgumentationTheory consists of an ArgumentationSystem and a knowledge
    base.
    ...
    def arguments(self) -> Dict[Literal, Set[InstantiatedArgument]]:
        """
        Get a dictionary of all arguments in this argumentation theory,
        indexed by their conclusion literal.

        :return: A dictionary of all arguments, indexed by their conclusion
        literal.
        """
    ...
    def _recompute_arguments(self):
        ...
        This step is necessary after every change in the argumentation system
        or knowledge base (note that this is done
        automatically by the corresponding setters).
        """
        arguments_per_conclusion = {
            literal: set()
            for literal in self._argumentation_system.language.values()}

        for knowledge_item in self._knowledge_base_axioms:
            arguments_per_conclusion[knowledge_item].add(
                InstantiatedArgument.axiom_based(knowledge_item))
        for knowledge_item in self._knowledge_base_ordinary_premises:
            arguments_per_conclusion[knowledge_item].add(
                InstantiatedArgument.ordinary_premise_based(knowledge_item))
            ...
            for defeasible_rule in self._argumentation_system.defeasible_rules:
                possible_antecedents = \
                    [[argument_for_antecedent
                      for argument_for_antecedent in
                      arguments_per_conclusion[antecedent]
                      if defeasible_rule.consequent not in
                      argument_for_antecedent.sub_conclusions]
                     for antecedent in defeasible_rule.antecedents]
                if all(possible_antecedents):
                    for direct_sub_argument_tuple in \
                            itertools.product(*possible_antecedents):
                        new_instantiated_argument = \
                            InstantiatedArgument.defeasible_rule_based(
                                defeasible_rule,
                                set(direct_sub_argument_tuple))
                        if new_instantiated_argument not in \
                                arguments_per_conclusion[
                                    defeasible_rule.consequent]:
                            arguments_per_conclusion[
                                defeasible_rule.consequent].add(
                                new_instantiated_argument)
                            change = True
            for strict_rule in self._argumentation_system.strict_rules:
                possible_antecedents = \
                    [[argument_for_antecedent
                      for argument_for_antecedent
                      in arguments_per_conclusion[antecedent]
                      if strict_rule.consequent not in
                      argument_for_antecedent.sub_conclusions]
                     for antecedent in strict_rule.antecedents]
                if all(possible_antecedents):
                    for direct_sub_argument_tuple in itertools.product(
                            *possible_antecedents):
                        new_instantiated_argument = \
                            InstantiatedArgument.strict_rule_based(
                                strict_rule, set(direct_sub_argument_tuple))
                        if new_instantiated_argument not in \
                                arguments_per_conclusion[
                                    strict_rule.consequent]:
                            arguments_per_conclusion[
                                strict_rule.consequent].add(
                                new_instantiated_argument)
                            change = True
    ...
    def all_arguments(self) -> List[InstantiatedArgument]:
        ...
        """
        return [argument for arguments_for_literal in self.arguments.values()
                for argument in arguments_for_literal]

    @property
    def all_attacks(self) -> \
            List[Tuple[InstantiatedArgument, InstantiatedArgument]]:
        """
    ...
    @staticmethod
    def rebuts_on_conclusion(argument_a: InstantiatedArgument,
                             argument_b: InstantiatedArgument) -> bool:
        """
        ...
        :return: Boolean indicating whether argument_a rebuts argument_b on
        its conclusion.
        """
        ...
        if not isinstance(argument_b.top_rule, DefeasibleRule):
            return False
        return argument_a.conclusion.is_contrary_or_contradictory_of(
            argument_b.conclusion)

    def rebuts(self, argument_a: InstantiatedArgument,
               argument_b: InstantiatedArgument) -> bool:
        """
    ...
    def rebuts_and_is_not_weaker(self, argument_a: InstantiatedArgument,
                                 argument_b: InstantiatedArgument,
                                 ordering: Ordering) -> bool:
        """
        Check if argument_a rebuts argument_b and is not weaker according to
        the given ordering.

        :param argument_a: The supposedly attacking argument.
        :param argument_b: The supposedly attacked argument.
        :param ordering: The ordering used to identify argument strength.
        :return: Boolean indicating whether argument_a rebuts argument_b and
        is stronger.
        """
        return any([self.rebuts_on_conclusion(argument_a, sub_argument_b) and
                    not ordering.argument_is_strictly_weaker_than(
                        argument_a, sub_argument_b)
                    for sub_argument_b in argument_b.sub_arguments])

    @staticmethod
    def contrary_rebuts_on_conclusion(
        ...
        :param argument_a: The supposedly attacking argument.
        :param argument_b: The supposedly attacked argument.
        :return: Boolean indicating whether argument_a contrary-rebuts
        argument_b on its conclusion.
        """
        if argument_b.is_observation_based:
            return False
        if not isinstance(argument_b.top_rule, DefeasibleRule):
    ...
    def contrary_rebuts(self, argument_a: InstantiatedArgument,
                        argument_b: InstantiatedArgument) -> bool:
        """
        ...
        argument_b.
        """
        return any([self.contrary_rebuts_on_conclusion(argument_a,
                                                       sub_argument_b)
                    for sub_argument_b in argument_b.sub_arguments])
    ...
    def undercuts_on_top_rule(
        ...
        :param argument_a: The supposedly attacking argument.
        :param argument_b: The supposedly attacked argument.
        :return: Boolean indicating whether argument_a undercuts argument_b on
         its top rule.
        """
        if argument_b.is_observation_based:
            return False
        if not isinstance(argument_b.top_rule, DefeasibleRule):
            return False
        argument_b_top_rule_literal = self._argumentation_system.get_literal(
            argument_b.top_rule)
        return argument_a.conclusion.is_contrary_or_contradictory_of(
            argument_b_top_rule_literal)

    def undercuts(self, argument_a: InstantiatedArgument,
                  argument_b: InstantiatedArgument) -> bool:
        """
    ...
    @staticmethod
    def undermines(argument_a: InstantiatedArgument,
                   argument_b: InstantiatedArgument) -> bool:
        """
        ...
        """
        return any([argument_a.conclusion.is_contrary_or_contradictory_of(
            ordinary_premise_b)
                    for ordinary_premise_b in argument_b.ordinary_premises])

    @staticmethod
    def undermines_and_is_not_weaker(argument_a: InstantiatedArgument,
                                     argument_b: InstantiatedArgument,
                                     ordering: Ordering) -> bool:
        """
        Check if argument_a undermines argument_b and is not weaker according
        to the specifies ordering.

        :param argument_a: The supposedly attacking argument.
        :param argument_b: The supposedly attacked argument.
        :param ordering: The ordering used to decide if argument_a is weaker
        than argument_b.
        :return: Boolean indicating whether argument_a undermines argument_b
        and is not weaker.
        """
        return any([argument_a.conclusion.is_contrary_or_contradictory_of(
            sub_argument_b.conclusion) and
                    not ordering.argument_is_strictly_weaker_than(
                        argument_a, sub_argument_b)
                    for sub_argument_b in argument_b.sub_arguments
                    if sub_argument_b.is_observation_based and
                    sub_argument_b.is_plausible])

    @staticmethod
    def contrary_undermines(
            argument_a: InstantiatedArgument,
            argument_b: InstantiatedArgument) -> bool:
        """
        ...
        :param argument_a: The supposedly attacking argument.
        :param argument_b: The supposedly attacked argument.
        :return: Boolean indicating whether argument_a
        contrary-undermines argument_b.
        """
        return any([argument_a.conclusion.is_contrary_of(ordinary_premise_b)
                    for ordinary_premise_b in argument_b.ordinary_premises])

    def attacks(self, argument_a: InstantiatedArgument,
                argument_b: InstantiatedArgument):
        """
        ...
        """
        return self.rebuts(argument_a, argument_b) or \
            self.undercuts(argument_a, argument_b) or \
            self.undermines(argument_a, argument_b)

    def defeats(self, argument_a: InstantiatedArgument,
                argument_b: InstantiatedArgument, ordering: Ordering) -> bool:
        """
    ...
    def recompute_all_defeats(self, ordering: Ordering) -> List[Defeat]:
        """
        Recompute all defeats between all arguments, given the specified
        ordering.

        :param ordering: The ordering used to decide if the attacking argument
        is weaker than the attacking argument.
    ...
    def create_abstract_argumentation_framework(
            self, name: str, ordering: Optional[Ordering] = None):
        """
        Create an abstract argumentation framework based on this argumentation
        theory. Note: if no ordering is given,
        last link elitist ordering is chosen as default ordering.

        :param name: The name of the argumentation framework.
        :param ordering: Ordering that influences which attacks are defeats.
        Note: default is last link elitist.
        :return: Abstract argumentation framework based on this argumentation
        theory.

        """
        if ordering is None:
            ordering = LastLinkElitistOrdering(
                self.argumentation_system.rule_preferences,
                self.ordinary_premise_preferences)
        return AbstractArgumentationFramework(
            name, self.all_arguments, self.recompute_all_defeats(ordering))

    def create_structured_argumentation_framework(
            self, name: str, ordering: Optional[Ordering] = None):
        """
        Create a structured argumentation framework based on this
        argumentation theory. Note: if no ordering is given,
        last link elitist ordering is chosen as default ordering.

        :param name: The name of the argumentation framework.
        :param ordering: Ordering that influences the argument preferences.
        Note: default is last link elitist.
        :return: Structured argumentation framework based on this argumentation
        theory.
        ...
        return StructuredArgumentationFramework(
            name, self.all_arguments, self.all_attacks,
            [(arg_a, arg_b)
