...
class ArgumentationTheoryGenerator:
    def __init__(self, argumentation_system: ArgumentationSystem,
                 knowledge_literal_ratio: float,
                 axiom_knowledge_ratio: float):
        self.argumentation_system = argumentation_system

        if knowledge_literal_ratio > 0.5:
            raise ValueError('At most half of the literals can be in the '
                             'knowledge base.')
        if knowledge_literal_ratio < 0:
            raise ValueError('The knowledge literal ratio should be at least '
                             'one.')
    ...
    def generate(self) -> ArgumentationTheory:
        normal_literals = [
            literal
            for literal_str, literal in
            self.argumentation_system.language.items()
            if 'd' not in literal_str]
        nr_of_literals = len(normal_literals)
        knowledge_base_size = \
            int(self.knowledge_literal_ratio * nr_of_literals)
        ...
        for _ in range(knowledge_base_size):
            ...
            for new_knowledge_contrary in \
                    new_knowledge.contraries_and_contradictories:
        ...
        ordinary_premises = [knowledge_base[i]
