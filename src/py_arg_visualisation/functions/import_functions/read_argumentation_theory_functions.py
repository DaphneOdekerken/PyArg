...
def _read_axioms_and_ordinary_premises(axioms_or_ordinary_premises_str) -> \
        List[str]:
    return [literal_str.strip()
            for literal_str in axioms_or_ordinary_premises_str.split()]


def _read_strict_rules(strict_rules_str) -> List[Tuple[str, Set[str], str]]:
    def read_strict_rule(rule_id, strict_rule_str) -> \
            Tuple[str, Set[str], str]:
        if '->' not in strict_rule_str:
            raise ValueError(
                'Each strict rule should contain -> at least once.')
        before_rule, after_rule = strict_rule_str.split('->', 1)
        antecedents = set(antecedent_str.strip()
                          for antecedent_str in before_rule.split(','))
        if '->' in after_rule:
    ...
    return [read_strict_rule(str(rule_id), strict_rule_str)
            for rule_id, strict_rule_str in enumerate(
            strict_rules_str.splitlines())]


def _read_defeasible_rules(defeasible_rules_str: str) -> \
        List[Tuple[str, Set[str], str]]:
    def read_defeasible_rule(named_defeasible_rule_str: str) -> \
            Tuple[str, Set[str], str]:
        ...
        if '=>' not in defeasible_rule_str:
            raise ValueError(
                'Each defeasible rule should contain => at least once.')
        ...
        if '=>' in after_rule:
            raise ValueError(
                'Each defeasible rule should contain => only once.')
        consequent = after_rule.strip()
...
def _read_preferences(preferences_str: str) -> List[Tuple[str]]:
    ...
    return [read_preference_rule(preference_str_rule)
            for preference_str_rule in preferences_str.splitlines()]


def read_argumentation_theory(axioms_str: str, ordinary_premises_str: str,
                              strict_rules_str: str, defeasible_rules_str: str,
                              premise_preferences_str: str,
                              defeasible_rule_preference_str: str):
    """
    Calculate the argumentation theory from the axioms, ordinary premises,
    strict and defeasible rules, premise and
    rule preference and the given ordering

    :param axioms_str: The provided axioms (premises that cannot be attacked).
    :param ordinary_premises_str: The ordinary premises (premises that can be
    questioned).
    :param strict_rules_str: The provided strict rules (rules that cannot be
    attacked).
    :param defeasible_rules_str: The defeasible rules (rules that can be
    questioned).
    :param premise_preferences_str: The preferences over the ordinary premises.
    :param defeasible_rule_preference_str: The preferences over the defeasible
     rules.
    """

    # Read axioms, ordinary premises, defeasible rules and strict rules
    # (first in a str format) from the strs
    axioms = _read_axioms_and_ordinary_premises(axioms_str)
    ordinary_premises = _read_axioms_and_ordinary_premises(
        ordinary_premises_str)
    ...
    ordinary_premise_preference_strs = \
        _read_preferences(premise_preferences_str)
    ordinary_premise_preferences = \
        PreferencePreorder.create_reflexive_preorder(ordinary_premises)
    ...
    defeasible_rule_preference_strs = \
        _read_preferences(defeasible_rule_preference_str)
    defeasible_rule_preferences = \
        PreferencePreorder.create_reflexive_preorder(defeasible_rules)
    ...
    argumentation_system = ArgumentationSystem(
        language, contraries_and_contradictories, strict_rules,
        defeasible_rules, defeasible_rule_preferences)
    argumentation_theory = ArgumentationTheory(
        argumentation_system, axioms, ordinary_premises,
        ordinary_premise_preferences)
