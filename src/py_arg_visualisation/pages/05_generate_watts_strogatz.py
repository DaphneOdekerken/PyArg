...
left_column = dbc.Col([
    ...
    dbc.Row([dbc.Col(html.B('Use Seed')),
             dbc.Col(dbc.Select(
                 ...
                 value='No', id='05-generate-watts-strogatz-use-seed-input'))]),
...
@callback(Output('05-generate-watts-strogatz-output', 'children'),
          ...
          State('05-generate-watts-strogatz-nr-join-neighbours-input', 'value'),
          State('05-generate-watts-strogatz-seed-input', 'value'),
          State('05-generate-watts-strogatz-use-seed-input', 'value'),
          State('05-generate-watts-strogatz-prob-defeat-rewiring-input', 'value'),
          State('05-generate-watts-strogatz-prob-cycle-input', 'value'))
def generate_abstract_argumentation_framework(
        nr_clicks: int, nr_arguments: str,nr_join_neighbours: str, seed: str,use_seed: str, prob_rewiring: str, prob_cycle: str):
    if not nr_clicks:
    ...
    generator = WattsStrogatz(nr_of_arguments=nr_arguments_int,nr_join_neighbours=nr_join_neighbours_int,prob_edge_rewiring=prob_rewiring_float,prob_cycle=prob_cycle_float,seed=seed_int)
        ...
        [html.H2('Output'),
         dbc.Row([
             dbc.Col([html.B('Arguments'), dbc.Textarea(
                 ...
                 style={'height': '300px'},
                 id={'type': '05-generate-watts-strogatz-arguments-text', 'index': nr_clicks})]),
             dbc.Col([html.B('Defeats'), dbc.Textarea(
                 ...
                 style={'height': '300px'},
                 id={'type': '05-generate-watts-strogatz-defeats-text', 'index': nr_clicks})])
         ]),
         html.Br(),
         dbc.Row([
             dbc.InputGroup([
                 dbc.InputGroupText('Filename'),
                 dbc.Input(value='generated_ws_af', id={'type': '05-generate-watts-strogatz-filename', 'index': nr_clicks}),
                 dbc.InputGroupText('.'),
                 dbc.Select(options=[{'label': extension, 'value': extension}
                                     for extension in ['JSON', 'TGF', 'APX', 'ICCMA23']],
                            value='JSON', id={'type': '05-generate-watts-strogatz-extension', 'index': nr_clicks}),
                 dbc.Button('Download', id={'type': '05-generate-watts-strogatz-download-button', 'index': nr_clicks}),
             ]),
             dcc.Download(id={'type': '05-generate-watts-strogatz-downloader', 'index': nr_clicks})
         ])
         ]
...
@callback(
    Output({'type': '05-generate-watts-strogatz-downloader', 'index': MATCH}, 'data'),
    Input({'type': '05-generate-watts-strogatz-download-button', 'index': MATCH}, 'n_clicks'),
    State({'type': '05-generate-watts-strogatz-arguments-text', 'index': MATCH}, 'value'),
    State({'type': '05-generate-watts-strogatz-defeats-text', 'index': MATCH}, 'value'),
    State({'type': '05-generate-watts-strogatz-filename', 'index': MATCH}, 'value'),
    State({'type': '05-generate-watts-strogatz-extension', 'index': MATCH}, 'value'),
