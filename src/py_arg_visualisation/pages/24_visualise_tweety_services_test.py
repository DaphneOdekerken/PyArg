...
from py_arg.abstract_argumentation.classes. \
    abstract_argumentation_framework import AbstractArgumentationFramework
...
from py_arg_visualisation.functions.tweety_services_functions import \
    tweety_services_handler

dash.register_page(__name__, name='Visualise Tweety Services Test',
                   title='Visualise Tweety Services')
...
def get_abstract_setting_specification_div():
    return html.Div(children=[
        ...
        dbc.Col([
            dbc.Row([dbc.Col(
                dbc.Button('Generate random', id='generate-random-af-button',
                           n_clicks=0,
                           className='w-100')),
                dbc.Col(dcc.Upload(
        ...
        html.Div([
            dbc.Row([
                ...
                dbc.Col(dbc.Select(options=semantics_mapping,
                                   value='wad',
                                   id='tweety-evaluation-semantics')),
            ]),
...
left_column = dbc.Col(
    dbc.Accordion([
        ...
        dbc.AccordionItem(get_abstract_evaluation_div(), title='Evaluation',
                          item_id='Evaluation'),
        dbc.AccordionItem(get_abstract_explanation_div(), title='Explanation',
                          item_id='Explanation')
...
def generate_abstract_argumentation_framework(_nr_of_clicks_random: int,
                                              af_content: str,
                                              af_filename: str):
    """
    ...
    if dash.callback_context.triggered_id == 'generate-random-af-button':
        random_af = AbstractArgumentationFrameworkGenerator(8, 8,
                                                            True).generate()
        abstract_arguments_value = '\n'.join(
            (str(arg) for arg in random_af.arguments))
...
def create_abstract_argumentation_framework(arguments: str, attacks: str,
    ...
    if dash.callback_context.triggered_id != \
            'tweety-selected-argument-store-abstract':
...
def download_generated_abstract_argumentation_framework(
    ...
    if extension == 'JSON':
        argumentation_framework_json = ArgumentationFrameworkToJSONWriter(). \
            to_dict(argumentation_framework)
...
def evaluate_abstract_argumentation_framework(arguments: str, attacks: str,
    ...
    if TWEETY_SERVICES_UP:
        ...
        arguments_id_mapping = dict(
            zip(arguments_list, range(1, len(arguments_list) + 1)))
        ...
        attacks_id_mapped = [
            [arguments_id_mapping[att[0]], arguments_id_mapping[att[1]]] for
            att
            in attacks_list]
        ...
        if status == 'SUCCESS':
            try:
                extensions = ast.literal_eval(
                    tweety_service_response[0]['answer'])
                if not (isinstance(extensions, list) and all(
                        isinstance(item, set) for item in extensions)):
            ...
            id_arguments_mapping = {v: Argument(k) for k, v in
                                    arguments_id_mapping.items()}
            extensions = [set(map(id_arguments_mapping.get, ext)) for ext in
                          extensions]
    else:
    ...
    if status == 'SUCCESS':
        for extension in sorted(extensions):
            out_arguments = {
                attacked for attacked in arg_framework.arguments
                if any(
                    argument in arg_framework.get_incoming_defeat_arguments(
                        attacked)
                    for argument in extension)}
            undecided_arguments = {
                argument for argument in arg_framework.arguments
                if argument not in extension and argument not in out_arguments}
            extension_readable_str = '{' + ', '.join(
                argument.name for argument in sorted(extension)) + '}'
            extension_in_str = '+'.join(
                argument.name for argument in sorted(extension))
...
def derive_explanations_abstract_argumentation_framework(
    ...
    return html.Div([html.Div(html.B('Explanation(s) by argument:'))] +
                    [html.Div([
                        ...
                        html.Ul([html.Li(
                            str(explanation_value).replace('set()', '{}'))
