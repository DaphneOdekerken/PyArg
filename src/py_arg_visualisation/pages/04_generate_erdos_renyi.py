...
from py_arg.generators.abstract_argumentation_framework_generators.abstract_argumentation_framework_generator import \
    AbstractArgumentationFrameworkGenerator, ErdosRenyiGenerator
...
def generate_abstract_argumentation_framework(
        nr_clicks: int, nr_arguments: str, prob_defeats: str, is_directed: str, seed: str,use_seed: str):
    if not nr_clicks:
    ...
    # )
    generator = ErdosRenyiGenerator(nr_of_arguments=nr_arguments_int,prob_edge=prob_defeats_float,is_directed=is_directed_bool,seed=seed_int)
        ...
        [html.H2('Output'),
         dbc.Row([
             dbc.Col([html.B('Arguments'), dbc.Textarea(
                 ...
                 style={'height': '300px'},
                 id={'type': '04-generate-abstract-arguments-text', 'index': nr_clicks})]),
             dbc.Col([html.B('Defeats'), dbc.Textarea(
                 ...
                 style={'height': '300px'},
                 id={'type': '04-generate-abstract-defeats-text', 'index': nr_clicks})])
         ]),
         html.Br(),
         dbc.Row([
             dbc.InputGroup([
                 dbc.InputGroupText('Filename'),
                 dbc.Input(value='generated_er_af', id={'type': '04-generate-abstract-filename', 'index': nr_clicks}),
                 dbc.InputGroupText('.'),
                 dbc.Select(options=[{'label': extension, 'value': extension}
                                     for extension in ['JSON', 'TGF', 'APX', 'ICCMA23']],
                            value='JSON', id={'type': '04-generate-abstract-extension', 'index': nr_clicks}),
                 dbc.Button('Download', id={'type': '04-generate-abstract-download-button', 'index': nr_clicks}),
             ]),
             dcc.Download(id={'type': '04-generate-abstract-downloader', 'index': nr_clicks})
         ])
         ]
...
@callback(
    Output({'type': '04-generate-abstract-downloader', 'index': MATCH}, 'data'),
    Input({'type': '04-generate-abstract-download-button', 'index': MATCH}, 'n_clicks'),
    State({'type': '04-generate-abstract-arguments-text', 'index': MATCH}, 'value'),
    State({'type': '04-generate-abstract-defeats-text', 'index': MATCH}, 'value'),
