...
def apply(extension_set: Set) -> AbstractArgumentationFramework:
    ...
    arguments = canon_cf.arguments
    defeats = canon_cf.defeats
    for el in to_elim:
        new_arg = Argument(str(set(el)))
        arguments.add(new_arg)
        defeats.add(Defeat(new_arg, new_arg))
        for arg in canon_cf.arguments.difference(el):
            defeats.add(Defeat(arg, new_arg))

    return AbstractArgumentationFramework('', arguments=arguments, defeats=defeats)
